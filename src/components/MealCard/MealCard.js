      ...
      <div className="icons-container">
        {isInPlanner && (
          <div className="option">
            ...
            />
            <h6 className="meal-card-option-header">Done</h6>
          </div>
        )}
        {!isPlanned && (
          <div className="option">
            <AiFillFileAdd
              className="add-toPlan-btn"
              onClick={() => handleAdd(meal.id)}
            />
            <h6 className="meal-card-option-header">Plan me !</h6>
          </div>
        )}
        {isPlanned && !isInPlanner && (
          <div className="option">
            <GoChecklist className="added-toPlan-btn" />
            <h6 className="meal-card-option-header">Im already planned</h6>
          </div>
        )}
        {!isFav && !isInPlanner && (
          <div className="option">
            <AiFillStar
              className="add-toFav-btn"
              onClick={() => handleFav(meal.id, true)}
            />
            <h6 className="meal-card-option-header">Add me to Favourites</h6>
          </div>
        )}
        {isFav && !isInPlanner && (
          <div className="option">
            <AiFillStar
              ...
              onClick={() => handleFav(meal.id, false)}
            />
            <h6 className="meal-card-option-header">I'm favourite!</h6>
          </div>
        )}
        <div className="option">
          ...
          <h6 className="meal-card-option-header">details</h6>
        </div>
      </div>
    </motion.div>
  );
